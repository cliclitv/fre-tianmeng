...
export default function Register({ id }) {
    ...
    async function register() {
        ...
        setLoading(true)
        const res = await post("https://www.tmread.com/user/register", { name, pwd, qq, time: 0, sign: "这个人很懒，什么都没有留下~" })
        setLoading(false)
        if (res.code === 200) {
            alert("注册成功啦~")
        } else {
    ...
    function logout() {
        localStorage.clear()
        window.location.href = 'https://www.tmread.com'
    }
    return <div class="login">
        ...
        {id && <li><button onClick={logout} style={{ background: 'var(--cyan)' }}>退出登陆</button></li>}
